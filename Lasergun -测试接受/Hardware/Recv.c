...
#define RECV_PORT    GPIOA
#define RECV_PIN     GPIO_Pin_2
...
static uint8_t Data_ReadBit(void) {
	...
	votes[0] = GPIO_ReadInputDataBit(RECV_PORT, RECV_PIN);
	
	Delay_us(BIT_TIME_US / 4);
	votes[1] = GPIO_ReadInputDataBit(RECV_PORT, RECV_PIN);
	
	Delay_us(BIT_TIME_US / 4);
	votes[2] = GPIO_ReadInputDataBit(RECV_PORT, RECV_PIN);
	
	Delay_us(BIT_TIME_US / 4);
    uint8_t raw = GPIO_ReadInputDataBit(RECV_PORT, RECV_PIN);
...
void Recv_Init(void) {
    ...
    // 2) PA2 
    GPIO_InitStruct.GPIO_Pin  = RECV_PIN;
    GPIO_InitStruct.GPIO_Mode = GPIO_Mode_IN_FLOATING;
    GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(RECV_PORT, &GPIO_InitStruct);
